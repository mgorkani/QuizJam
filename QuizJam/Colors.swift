//
//  Colors.swift
//  QuizJam
//
//  Created by Chaithra Pabbathi on 2/19/21.
//

import SwiftUI

extension Color {
//    static let fontColor = Color(#colorLiteral(red: 0.2468928397, green: 0.266464293, blue: 0.234403491, alpha: 1))
    static let fontColor = Color(#colorLiteral(red: 0.9174692631, green: 0.8395887613, blue: 0.6754086018, alpha: 1))
    static let fontShadowColor = Color(#colorLiteral(red: 1, green: 0.9282022119, blue: 0.7584571838, alpha: 1))
    static let cardBackgroundColor = Color(#colorLiteral(red: 0.8363150954, green: 0.3524754047, blue: 0.2880167663, alpha: 1))
    static let cardBorderBackgroundColor = Color(#colorLiteral(red: 0.9548953176, green: 0.4006324708, blue: 0.3307930231, alpha: 1))
    
    static let completionBarBackgroundColor = Color(#colorLiteral(red: 1, green: 0.9282022119, blue: 0.7584571838, alpha: 1))
    static let completionBarButtonColor = Color(#colorLiteral(red: 1, green: 0.4447793961, blue: 0.3672052324, alpha: 1))
    
//    static let cardBackgroundColor = Color(#colorLiteral(red: 0.9436573386, green: 0.9879108071, blue: 0.7188943028, alpha: 1))
//    static let cardBackgroundColor = Color(#colorLiteral(red: 0.7432169914, green: 0.9934846759, blue: 0.957719028, alpha: 1))
//    static let cardBackgroundColor = Color(#colorLiteral(red: 0.9971958995, green: 0.9431241751, blue: 0.9527503848, alpha: 1))
//    static let cardBackgroundColor = Color(#colorLiteral(red: 0.8927761912, green: 0.9636031985, blue: 0.8475902081, alpha: 1))
}
